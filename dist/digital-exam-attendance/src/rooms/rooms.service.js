...
let RoomsService = class RoomsService {
    ...
    async createRoom(dto, userId, fullName) {
    ...
    async getRoomByCodeOrName(identifier) {
        const room = await this.roomRepository.findOne({
            where: [
        ...
        if (!room) {
            throw new common_1.NotFoundException(`Room with code or name "${identifier}" not found`);
        }
        return room;
    }
